...
// The curtains color is represented as a string(str) consisting of a’s and b’s of length N. Then, they are packed (substring) into L 
// number of curtains in each box. The box with the maximum number of ‘aqua’ (a) color curtains is labeled. The task here is to find the 
...
public class MaxA {
    public static void solve(String str, int n) {
        ...
        for (int i = 0; i < arr.length; i += n) {
            int count = 0;
            for (int j = i; j < n + i && j < arr.length; j++) {
                char ch = arr[i];
                if (ch == 'a')
                    count++;
            }
